...
public class RoadComponent extends JComponent {
    ...
    public void update(double slowdown, double arrival) {
        ...
        freeway.paint(g, row, DOTSIZE, XDOTDIST);
        g.dispose();
        repaint();
    }
...
class Road {
    ...
    public void update(double prod_slowdown, double prod_create) {
        int i = 0;
        while (i < LENGTH && speed[i] == -1) {
            i++;
        }
        while (i < LENGTH) {
            if (Math.random() <= prod_slowdown && speed[i] > 0) {
                speed[i]--;
            } else if (speed[i] < MAXSPEED) {
                speed[i]++;
            }
            int inext = i + 1;
            while (inext < LENGTH && speed[i] == -1) {
                inext++;
            }
            if (inext < LENGTH) {
                if (speed[i] >= inext - i) {
        ...
        if (Math.random() <= prod_create && speed[0] == -1) {
